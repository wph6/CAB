...
class Block(nn.Module):
    ...
    def forward(
        ...
        teacher_act=None, return_act=None
    ):
        ...
        if not self.fused_add_norm:
            if residual is None:
                ...
                # residual = residual + drop_masks * hidden_states
        ...
        hidden_states = self.mixer(hidden_states, inference_params=inference_params, teacher_act=teacher_act, return_act=return_act)

        if return_act is not None:
            hidden_states, act = hidden_states
            return hidden_states, residual, act
...
def create_block(
    ...
    # import ipdb; ipdb.set_trace()
    mixer_cls = partial(Mamba, d_state=d_state, layer_idx=layer_idx, bimamba_type=bimamba_type, if_divide_out=if_divide_out, init_layer_scale=init_layer_scale,mode=mode,**ssm_cfg, **factory_kwargs)
    norm_cls = partial(
...
class VisionMamba(nn.Module):
    ...
    def forward_features(self, x, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False,
        ...
        if not self.if_bidirectional:
            ...
            for i, layer in enumerate(self.layers):
                ...
                if return_act is not None:
                    hidden_states, residual,act = layer(
                    hidden_states, residual, inference_params=inference_params,return_act=return_act)
                    acts.append(act)
                elif teacher_act is not None:
                    hidden_states, residual = layer(
                    hidden_states, residual, inference_params=inference_params,teacher_act=teacher_act[i])
                else:
                    hidden_states, residual = layer(
                    hidden_states, residual, inference_params=inference_params)
        else:
        ...
        if self.if_cls_token:
            if self.use_double_cls_token:
                pooled = (hidden_states[:, token_position[0], :] + hidden_states[:, token_position[1], :]) / 2
            else:
                pooled = hidden_states[:, token_position, :]
                # if self.use_middle_cls_token:
                ...
                # else:
                #     return hidden_states[:, token_position, :]
            return (pooled, acts) if return_act is not None else pooled
    ...
    def forward(self, x, return_features=False, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False, teacher_act=None, return_act=None):
        x = self.forward_features(x, inference_params, if_random_cls_token_position=if_random_cls_token_position, if_random_token_rank=if_random_token_rank,teacher_act=teacher_act, return_act=return_act)
        if return_act is not None:
            x, act = x
...
class DualVisionMamba(nn.Module):
    """
    Vision Transformer with support for global average pooling
    Has two streams (one is a teacher, the other is a student)
    """
    def __init__(self, mode=None, drop_path_rate=0,
        ...
        self.teacher = VisionMamba(mode='teacher', **teacher_kwargs)
        self.student = VisionMamba(mode=mode, **student_kwargs)
        self.teacher.default_cfg = _cfg()
        self.student.default_cfg = _cfg()
        self.dpr = [x.item() for x in torch.linspace(0, self.drop_path_rate, self.teacher_depth+1)]
...
def dual_vim_tiny_patch16(drop_path_rate=0, end_layer=0, **kwargs):
    kwargs = dict(
    ...
    model = DualVisionMamba(drop_path_rate=drop_path_rate, end_layer=end_layer,
